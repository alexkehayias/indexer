...
use crate::aql;
use crate::chat::{chat, chat_stream, find_chat_session_by_id, insert_chat_message};
...
async fn chat_stream_handler(
    ...
    let sse_stream = UnboundedReceiverStream::new(rx).map(|chunk| {
...
async fn send_notification(
    ...
    let payload =
        PushNotificationPayload::new("Notification", &payload.message, None, None, Some("indexer_updated"));
...
pub fn app(shared_state: Arc<RwLock<AppState>>) -> Router {
    ...
    Router::new()
        ...
        // Chat with notes
        .route("/notes/chat", post(chat_stream_handler))
